...
#If (WinActive("ahk_exe TheDivision2.exe") or debug) AND vShootMode=2 and _autofire and vAutoShoot
    ~RButton:: lessrecoil_triggerbot()	
~RButton Up::
    loop,4{
        Sleep 120
        sendkey("j",25,"up")
...
MenuHandler(menuname:="",submenuname:=""){
    menuname:=menuname?A_ThisMenu:%menuname%
    submenuname:=submenuname?A_ThisMenuItem:%submenuname%

    vmenu:=menuname="Tray"?submenuname:menuname
    ;如果是主选项
    ...
    case "自动连射|F4开关":
        {
            ...
            vStr:=vShootMode=1?"简单模式":"高级模式"

                if vTtsSpeak
                    Speak(_autofire ? "已开启自动连射" vStr: "已关闭自动连射" vStr)
    ...
    case "MySubMenu1": ;连射模式
        {
            if(submenuname="简单模式" or (submenuname=""?vShootMode=1:0))
            {
                Menu, MySubMenu1, Check, 简单模式
                ...
                if vTtsSpeak
                    Speak(vShootMode=1 ? "已切换简单模式" : )

                IniWrite, % vShootMode, TheDivision2.ini, 自动射击, 射击模式
            }
            else if(submenuname="高级模式" or (submenuname=""?vShootMode=2:0))
            {
                Menu, MySubMenu1, Check, 高级模式
    ...
    case "无限溜溜球|F5开关":
        {
            ...
            vAutoLiu := !vAutoLiu
            vStr := vLiuMode=1 ? "装配工模式" : "固线模式"
                if vTtsSpeak
                    Speak(vAutoLiu ? "已开启无限溜溜球 " vStr : "已关闭无限溜溜球" vStr)
    ...
    case "MySubMenu2": ; 溜溜球模式
        {
            if(submenuname="装配工模式" or (submenuname=""?vLiuMode=1:0)){
                Menu, MySubMenu2, Check, 装配工模式
                ...
                if vTtsSpeak
                    Speak(vLiuMode=1 ? "已切换装配工模式" : )
                IniWrite, % vAutoLiu, TheDivision2.ini, 无限溜溜球, 启用
            }
            else if(submenuname="固线模式" or (submenuname=""?vLiuMode=2:0))
            {
                Menu, MySubMenu2, Check, 固线模式
    ...
    case "MySubMenu3": ; 信息显示|F2
        {
            if(submenuname="渐隐模式" or (submenuname=""?vInfoMethod=1:0))
            {
                Menu , MySubMenu3, check, 渐隐模式
                ...
                if vTtsSpeak
                    Speak(vInfoMethod=1 ? "信息显示已切换渐隐模式" : )

                IniWrite , vInfoMethod, TheDivision2.ini, 信息显示, 显示模式 ;1-渐隐,2固定
            }
            else if(submenuname="固定模式" or (submenuname=""?vInfoMethod=2:0)){
                Menu , MySubMenu3, check, 固定模式
                ...
                if vTtsSpeak
                    Speak(vInfoMethod=2 ? "信息显示已切换固定模式" : )
    ...
    case "MySubMenu4": ; 键鼠操作模式
        {
            if(submenuname="兼容模式" or (submenuname=""?vajMode=1:0))
            {
                Menu , MySubMenu4, check, 兼容模式
            ...
            else if(submenuname="游戏模式" or (submenuname=""?vajMode=2:0)){
            ...
            else if(submenuname="强力模式" or (submenuname=""?vajMode=3:0)){
                Menu , MySubMenu4, uncheck, 兼容模式
                Menu , MySubMenu4, uncheck, 游戏模式
                Menu , MySubMenu4, check, 强力模式
...
infoMenu:
    ...
    if _autofire
    {
        info .="`n连射模式:`t- " transit(vShootMode,"fire") "`t`t- 双击F4"
    ...
    if vAutoLiu
    {
        info .="`n溜溜球模式:`t- " transit(vLiuMode,"liu") "`t`t- 双击F5"
        . "`n固线自动重置:`t- " transit(vLiuMode) "`t`t- 三连F5"
    }
    ...
    info .="`n显示模式:`t- " transit(vInfoMethod,"info") "`t`t- 双击F2"
