...
if FileExist("TheDivision2.ini")
{
    ...
    IniRead , vajlMode, TheDivision2.ini, 键鼠操作模式, 按键模式, 1
    ; IniRead , vPlayMode, TheDivision2.ini, 键鼠操作模式, 游戏模式, 0
    ; IniRead , vDdMode, TheDivision2.ini, 键鼠操作模式, 驱动模式, 0
...
Menu,MySubMenu4,Add,驱动模式,DdMode
if (vajlMode==1) {
    Menu , MySubMenu4, check, 兼容模式
} else if(vajlMode==2){
    Menu , MySubMenu4, check, 游戏模式
} else if(vajlMode==3){
    Menu , MySubMenu4, check, 驱动模式
}
...
NormalMode:
    ...
    Menu , MySubMenu4, uncheck, 驱动模式
    vajlMode := 1 ;1-兼容模式 2-游戏模式 3-驱动模式
    SendMode Event
    ; vPlayMode := 0
    ; vDdMode := 0

    if vTtsSpeak
        Speak(vajlMode==1 ? "助手按键已切换兼容模式" : )

    IniWrite, % vajlMode, TheDivision2.ini, 键鼠操作模式, 按键模式
    ; IniWrite, % vPlayMode, TheDivision2.ini, 键鼠操作模式, 游戏模式
    ; IniWrite, % vDdMode, TheDivision2.ini, 键鼠操作模式, 驱动模式
return
...
PlayMode:
    ...
    Menu , MySubMenu4, check, 游戏模式
    Menu , MySubMenu4, uncheck, 驱动模式
    vajlMode := 2 ;1-兼容模式 2-游戏模式 3-驱动模式
    SendMode InputThenPlay
    ; vPlayMode := 1
    ; vDdMode := 0

    if vTtsSpeak
        Speak(vajlMode == 2 ? "助手按键已切换游戏模式" : )
...
DdMode:
    ...
    Menu , MySubMenu4, uncheck, 游戏模式
    Menu , MySubMenu4, check, 驱动模式
    vajlMode := 3 ;1-兼容模式 2-游戏模式 3-驱动模式
    ; vNormalMode := 0
    ; vPlayMode := 0
    ; vDdMode := 1

    if vTtsSpeak
        Speak(vajlMode==3 ? "助手按键已切换驱动模式" : )
...
infoMenu:
    ...
    info .="`n键鼠操作模式:`t- " transit(vajlMode,"ajmode")
...
Else if (vajlMode==3){
    ...
    {
    case "down":
    key == "LButton"?DD._key_press(%key%, "Down"):DD._key_press(%key%, "Down")
    Sleep DelayTime
    return
case "up":
SendInput {%key% up}
